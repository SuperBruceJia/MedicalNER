...
class ChineseNER:
    ...
    def train(self):
        ...
        if self.transfer_learning == True:
            keep_grad = [
                ...
                "char_linear_lstm.weight",
                "char_linear_lstm.bias",
                "word_linear_lstm.weight",
                "word_linear_lstm.bias",
                "hidden2tag.weight",
                "hidden2tag.bias"
            ]
        ...
        # Use Adam Optimizer
        optimizer = optim.Adam(params=filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.learning_rate, weight_decay=self.weight_decay)
        ...
        for epoch in range(1, self.epochs+1):
            ...
            for batch in self.train_manager.get_batch():
                ...
                len_word_tensor = torch.tensor(len_word, dtype=torch.int64, device=device)
                len_char_tensor = torch.tensor(len_char, dtype=torch.int64, device=device)
    ...
    def evaluate(self):
        ...
        """
        chars, labels, words, len_words, len_chars = zip(*self.dev_batch.__next__())
        chars_tensor = torch.tensor(chars, dtype=torch.long, device=device)
        words_tensor = torch.tensor(words, dtype=torch.float, device=device)
        len_word_tensor = torch.tensor(len_words, dtype=torch.int64, device=device)
    ...
    def predict(self):
        ...
        while True:
            ...
            _, paths = self.model(characters=char_tensor, len_char=len_char, words=word_tensor, len_word=len_word)
