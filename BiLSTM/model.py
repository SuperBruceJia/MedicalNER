...
class BiLSTMCRF(nn.Module):
    ...
    def prediction(self, sentence, real_length):
        ...
        lstm_out, self.hidden = self.lstm(packed_word, self.hidden)
        ...
        logits = self.Leaky_ReLu(self.linear3(logits))
        logits = self.Dropout(logits)

        logits = self.Leaky_ReLu(self.linear4(logits))
        logits = self.Dropout(logits)
    ...
    def neg_log_likelihood(self, sentences, tags, length):
        ...
        total_leng = torch.zeros(1, device=device)
        ...
        for logit, tag, leng in zip(logits, tags, length):
            ...
            total_score += self.total_score(logit).to(device)

            total_leng += leng

        # Output the NLL Loss
        return (total_score - real_path_score) / total_leng
