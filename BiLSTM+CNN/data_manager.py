...
class DataManager:
    ...
    def load_char_data(self):
        ...
        # print("{} characters: {}".format(self.data_type, len(self.char_data)))
        # print("Character Vocabulary Size: {}".format(self.input_size))
        # print("Unique tag: {}\n".format(len(self.tag_map.values())))
    ...
    def prepare_batch(self):
        ...
        self.words = self.pad_word_data(self.word_data)
        self.words = np.array(self.words, dtype=object)
        self.words = np.expand_dims(self.words, axis=1)
        self.overall_data = np.concatenate([self.chars, self.words], axis=1).tolist()
